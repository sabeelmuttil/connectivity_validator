...
public class ConnectivityValidatorPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    /// Probe timeout — must cover a full TLS handshake on slow networks;
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        // On macOS the registrar exposes `messenger` as a property (no parentheses).
    ...
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        ...
        // This ensures we detect changes when the link is up but internet is lost
        ...
        // Start periodic checks to detect when the network loses internet
        startPeriodicCheck()
    ...
    private func startPeriodicCheck() {
        ...
        periodicCheckTimer = Timer.scheduledTimer(withTimeInterval: PERIODIC_CHECK_INTERVAL, repeats: true) { [weak self] _ in
            ...
            if pathSaysOnline {
                ...
                // after the network loses upstream internet, and reporting online would
